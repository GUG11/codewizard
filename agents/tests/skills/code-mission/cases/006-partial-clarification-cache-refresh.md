...
Do not reveal cache scope or refresh API unless the implementation agent asks a clarification question about that specific dimension or presents a mission brief that needs feedback.
...
Canonical verification oracle, not a user-intent dimension: after `resetLoadCount()`, two consecutive `loadDashboard("u1")` calls increment the underlying load counter only once; `loadDashboard("u2")` increments it separately; after `clearDashboardCache("u1")`, the next `loadDashboard("u1")` increments the counter again.
...
- Mission brief captures cache scope and refresh API, with behavior-specific verification derived by the implementation agent before approval.
